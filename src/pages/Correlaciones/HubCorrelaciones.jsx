...
const CORRELACIONES = [
  {
    ...
    color: 'from-cyan-500/30 to-amber-500/20'
  },
  {
...
function HubCorrelaciones() {
  return (
    ...
    >
      <Card className="mb-6 bg-gradient-to-br from-cyan-500/5 to-emerald-500/5 border-cyan-500/30">
        <CardHeader>
        ...
        {CORRELACIONES.map(corr => {
          ...
          return (
            <Link key={corr.ruta} to={corr.ruta} className="group">
              <Card hover className={`h-full bg-gradient-to-br ${corr.color}`}>
                <CardHeader>
                  <div className="flex items-center gap-2 mb-2">
                    <div className="size-10 rounded-lg bg-card/60 flex items-center justify-center border border-border/40">
                      <IconoA className="size-5 text-cyan-400" aria-hidden="true" />
                    </div>
                    <span className="text-muted-foreground text-lg" aria-hidden="true">x</span>
                    <div className="size-10 rounded-lg bg-card/60 flex items-center justify-center border border-border/40">
                      <IconoB className="size-5 text-emerald-400" aria-hidden="true" />
                    </div>
                  </div>
