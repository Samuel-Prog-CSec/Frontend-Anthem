...
function TarjetaEstadisticaDashboardImpl({ titulo, valor, subtitulo, icono, cargando }) {
  const IconoComponente = icono;

  return (
    <div className="relative group h-full rounded-xl border border-border bg-card transition-colors duration-300 hover:border-[var(--border-emphasis)]">
      {/* Acento vertical izquierdo (1px) que marca el card sin saturarlo */}
      <div className="absolute left-0 top-6 bottom-6 w-px bg-[var(--border-strong)]" aria-hidden="true" />
      ...
      <div className="p-6 pl-7">
        <div className="flex items-start justify-between gap-4 mb-6">
          ...
          <div className="shrink-0 size-9 rounded-lg flex items-center justify-center border border-border">
            <IconoComponente className="size-4 text-muted-foreground" aria-hidden="true" />
          </div>
        </div>
